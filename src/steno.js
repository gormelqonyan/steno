...
export const encode = function (message, image) {
    ...
    // Վերցնում էնք հաղորդագրությունը և ամեն նիշը վերածում էնք բիթերի
    ...
    // modeMessage ի մեջից վերցնում էնք բիթերը և նկարի պիկսելի գույները 4 հատը 1 փոխում էնք
    for (offset = 0; (offset + threshold) * 4 <= data.length && (offset + threshold) <= modMessage.length; offset += threshold) {
        ...
        for (i = 0; i < threshold && i + offset < modMessage.length; i += 1) {
            q = 0;
            // նկարի պիկսլեի գույնը ըստ պառզ թվի ենթարկում էնք փոփխության
            for (j = offset; j < threshold + offset && j < modMessage.length; j += 1) {
                q += modMessage[j]
                qS[i] = (255 - prime + 1) + (q % prime);
            }
        }
        // ստացված պիկսելների գույները տեղադրում էնք 3 հատ պիկսլեը 1
